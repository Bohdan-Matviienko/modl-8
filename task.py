...
import functools
import pickle  # ★ додано для серіалізації
...
DEFAULT_DB = "addressbook.pkl"  # ★ ім'я файлу за замовчуванням

def save_data(book: AddressBook, filename: str = DEFAULT_DB) -> None:
    """Серіалізуємо AddressBook у файл (pickle)."""
    with open(filename, "wb") as f:
        pickle.dump(book, f, protocol=pickle.HIGHEST_PROTOCOL)

def load_data(filename: str = DEFAULT_DB) -> AddressBook:
    """Десеріалізуємо AddressBook з файлу або створюємо нову, якщо файл відсутній."""
    try:
...
def main():
    # ★ ВІДНОВЛЕННЯ на старті
    book = load_data(DEFAULT_DB)
    print("Welcome to the assistant bot!")
    print("Дані завантажено з", DEFAULT_DB)
    ...
    try:
        ...
        # ★ ЗБЕРЕЖЕННЯ при виході (незалежно від причини)
        save_data(book, DEFAULT_DB)
        print("Дані збережено у", DEFAULT_DB)
